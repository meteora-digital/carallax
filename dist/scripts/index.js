...
var CarallaxController = /*#__PURE__*/function () {
  function CarallaxController() {
    ...
    this.canvas = new Canvas(); // Create a new buffer canvas

    this.buffer = new Canvas(); // The static layer

    this["static"] = new Image(); // The layers in the canvas

    this.layers = {}; // Some timeouts

    this.timeouts = {}; // The maximum dpr

    this.dpr = window.devicePixelRatio || 1; // A store for all our already solved calculations

    this.calculations = {}; // We will store some values here
    ...
    this.IntersectionObserver.observe(this.canvas.element); // The parallax settings
    ...
    this.ResizeObserver.observe(this.canvas.element); // Set a classname for the static layer

    this["static"].className = 'parallax-canvas--static'; // Keep track of everything going on
  ...
  _createClass(CarallaxController, [{
    ...
    value: function load() {
      ...
      this.status.layers += 1; // create a new image element

      var image = new Image(); // store the index

      var index = this.status.layers; // When the image loads, we need to add a new ParallaxLayer

      image.addEventListener('load', function () {
        // Tell the status that something is loaded
        _this2.status.loaded = true; // Add this layer to the layers object

        _this2.layers[index] = new ParallaxLayer(image, index); // Now we need to update some things

        _this2.resize(); // Enable drawing
        ...
        }, 100); // Update the depth of the image
      ...
      if (src instanceof Element) {
        ...
        src.setAttribute('viewBox', "0 0 ".concat(width, " ").concat(height)); // Set the height and width of the image

        image.width = parseInt(width) + 'px';
        image.height = parseInt(height) + 'px'; // btoa the element and set it as the source

        image.src = 'data:image/svg+xml;base64,' + btoa(src.outerHTML); // Remove the orginal element
    ...
    value: function draw() {
      ...
      if (window.pageYOffset + window.innerHeight > this.status.view.top && window.pageYOffset < this.status.view.bottom) {
        ...
        var percentScrolled = this.getScrollPercent(); // Draw each image to the buffer canvas;
    ...
    value: function resize() {
      ...
      this.timeouts['resize'] = setTimeout(function () {
        ...
        _this7.canvas.resize(_this7.canvas.element.clientWidth, _this7.canvas.element.clientHeight); // Tell the buffer to resize


        _this7.buffer.resize(_this7.canvas.element.clientWidth, _this7.canvas.element.clientHeight);

        _this7.status.depth = _this7.canvas.element.clientWidth / Math.log(_this7.canvas.element.clientWidth) / 100 * (_this7.settings.depth * _this7.canvas.dpr); // Loop through the layers and resize them too, they only need the width of the canvas as their height is calculated
        ...
        _this7.cache.canvasOffset = _this7.canvas.pageYOffset; // Make sure it still draws during the resize

        _this7.enable(); // Clear all the calculations
...
var ParallaxLayer = /*#__PURE__*/function () {
  function ParallaxLayer(image, index) {
    ...
    this.image = image; // The layer index

    this.index = index; // The visual depth of the layer

    this.depth = 0; // Create a new buffer canvas

    this.canvas = new Canvas(); // The vertical position of the image

    this.alignment = 0; // The perspective position of the layer

    this.perspective = 0; // The pixel height of the layer

    this.height = 0; // The pixel width of the layer

    this.width = 0; // The parallax position calculations
  ...
  _createClass(ParallaxLayer, [{
    ...
    value: function resize(width) {
      ...
      this.calculations = {}; // We want to maintain the aspect ratio so we work out the height manually

      this.canvas.resize(Math.round(width), Math.round(width / this.image.naturalWidth * this.image.naturalHeight)); // Work out the pixel height of the layer

      this.height = this.canvas.element.height * (1 / this.canvas.dpr); // Work out the pixel width of the layer

      this.width = this.canvas.element.width * (1 / this.canvas.dpr); // After we have resized the image, we want to update the canvas
...
var Canvas = /*#__PURE__*/function () {
  function Canvas() {
    ...
    this.element = document.createElement('canvas'); // The canvas context

    this.ctx = this.element.getContext('2d'); // The devicePixelRatio

    this.dpr = window.devicePixelRatio || 1; // The page offset
  ...
  _createClass(Canvas, [{
    ...
    value: function resize() {
      ...
      this.element.height = height * this.dpr; // Update the pageYOffset of the canvas
